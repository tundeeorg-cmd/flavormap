...
## Added 2026-09-12 — production statistics do not measure what people cook

| # | Limitation | Severity | Statement |
|---|---|---|---|
| L23 | Production volume is not local availability | **HIGH** | `crop_production` (migration 028, `flavormap_oae_production.csv`) records where กระเทียม/หอมแดง/มะพร้าวผลแก่/พริกไทย etc. are *grown*, not where they are *cooked with*. A province can grow almost none of an ingredient and still use it constantly, because ingredients travel through markets — and a province can be a major producer and export nearly all of it. **This caveat must exist before any figure sets production data next to ingredient-usage data**, and it already bears out in the real numbers: ศรีสะเกษ is the #1 producer of หอมแดง (83,996 tonnes, 2568) yet **zero** of its 6 food67 dishes name it, while เชียงใหม่, the #2 producer, also shows zero (`docs/decisions.md`, Task 4). Production rank is weak evidence for cooking practice in either direction and must never be read as a proxy for it |
